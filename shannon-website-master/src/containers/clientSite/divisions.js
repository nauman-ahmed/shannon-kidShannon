...
function Divisions(props) {
  ...
  useEffect(()=>{
    dispatch(artistKeyword({})).then((res)=>{
            ...
            <>
              {item.ImageData.length > 0 ?(<>
                <div className="divisiondivider grad">
                  <h2 className="divisionh2">{item.keyword.toUpperCase()} ARTISTS</h2>
                </div>
                <div id="w-node-f734ee66-0b58-4c14-e08b-49ceded015c9-84f2d081" className="_4cols divisions">
                  {item.ImageData.map((item1,key1)=>(
                    <>
                    {key1<=7?item1.artistId&&(<>
                      <Link key={key1} id="w-node-f734ee66-0b58-4c14-e08b-49ceded015ca-84f2d081" to="#" className="artistcard bipoc w-inline-block"><img src={String(item1.mainImage[0].subImage[0]&&item1.mainImage[0].subImage[0].path)} loading="lazy" alt="" className="image" />
